...
def create_quiz(video_transcription, AI71_API_KEY):
    if 'quiz_data_list' not in st.session_state or not st.session_state.quiz_data_list:
        with st.spinner("Crafting your quiz...🤓"):
            ...
            if quiz_data:
                ...
                st.error("Error in generating quiz data. Please try again.")
...
def display_quiz():
    ...
    with st.form(key='quiz_form'):
        for i, q in enumerate(st.session_state.quiz_data_list):
            ...
            st.session_state.user_answers[i] = response  # Update the stored answer right after fetching it
